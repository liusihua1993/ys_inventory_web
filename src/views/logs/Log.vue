...
export default {
  ...
  methods: {
    ...
    logList() {
      let x = `page=${this.page}&limit=${this.limit}&module_name=${this.formInline.templateName}&operator=${this.formInline.user}&operationTimeStart=${this.value4[0] || ''}&operationTimeEnd=${this.value4[1] || ''}`
      logList(x).then((res) => {
        let datas = res.data;
